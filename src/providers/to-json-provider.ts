...
export const toJsonRpcProvider = (providerOrUrl: Pick<ExternalProvider, 'request' | 'send'> | { currentProvider: any } | Web3Provider | JsonRpcProvider | string): JsonRpcProvider => {
  ...
  } else if ('currentProvider' in providerOrUrl) {
    return new Web3Provider(providerOrUrl.currentProvider)
  } else {
